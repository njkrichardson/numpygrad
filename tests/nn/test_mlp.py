...
def _linear_modules(sequential):
    """Collect Linear submodules from a Sequential in order. Linears are at indices
    0, 2, 4, ... (ReLU at 1, 3, ...).
    """
...
def configuration(draw):
    ...
    hidden_sizes = draw(
        st.lists(DIM_SIZE, min_size=MIN_NUM_HIDDEN_LAYERS, max_size=MAX_NUM_HIDDEN_LAYERS)
    )
...
def test_mlp_backward(config):
    ...
    yt = torch_mlp(x_t)
    torch_params = [p for layer in torch_mlp.layers for p in (layer.weight, layer.bias)]
    ...
    for ngp_layer, _ in zip(linears, torch_mlp.layers, strict=False):
        gwt = next(grad_iter)
        gbt = next(grad_iter)
